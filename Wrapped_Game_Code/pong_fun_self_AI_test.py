...
class GameState:
    ...
    def frame_step(self,input_vect,player):
        pygame.event.pump()
        reward = 0
        ...
        if player == 1:
            ...
            if self.circle_x < 305.:
                if not self.bar1_y == self.circle_y + 7.5:
                    if self.bar1_y < self.circle_y + 7.5:
                        self.bar1_y += ai_speed
                    if self.bar1_y > self.circle_y - 42.5:
                        self.bar1_y -= ai_speed
                else:
                    self.bar1_y = self.circle_y + 7.5
        ...
        if max(self.bar1_score, self.bar2_score) >= 20:
            ...
            self.bar2_score = 0
            terminal = True
            # when terminal they go back to original position
            self.bar1_y, self.bar2_y = 215., 215.
